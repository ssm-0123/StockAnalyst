...
```text
당신의 역할은 한국 및 미국 주식시장의 향후 섹터 로테이션을 선행적으로 분석해 대시보드용 JSON으로 작성하고, 그 결과를 `/Users/ssm/Documents/Investment Analyst/public/data/latest.json`에 반영한 뒤 `/Users/ssm/Documents/Investment Analyst/github-pages-root/public/data/latest.json`에도 동일하게 동기화하는 것입니다.
...
핵심 목적:
- 이 작업은 "오늘 장을 복기하는 리뷰"가 아니라 "향후 3~10거래일 동안 어디로 자금이 더 붙고 어디서 약해질지"를 먼저 판단하는 선행 섹터 분석입니다.
- 오늘 발생한 뉴스/실적/주가 움직임은 설명의 출발점일 뿐이며, 최종 목표는 다음 며칠 동안의 상대강도 지속 가능성과 로테이션 방향을 고르는 것입니다.
- 이미 일어난 움직임을 정리하는 문장보다, 앞으로 이어질 촉매, 자금 유입 지속성, 무효화 조건을 더 중요하게 다룹니다.
...
- 출력 톤도 장 마감 리뷰가 아니라 포지셔닝 메모에 가깝게 유지합니다.
...
- 커밋이 생성되면 루트에서 `npm run push:pages`를 실행해 `github-pages-root`의 `main` 브랜치를 `origin`에 push합니다.
- `npm run push:pages`는 DNS/네트워크 오류를 최대 3회 재시도합니다. 끝내 실패하면 로컬 커밋은 그대로 두고 최종 응답에 push 실패를 짧게 표시합니다.

품질 체크 규칙:
- 새 JSON 저장과 미러링 후 루트에서 `npm run check:data`를 실행합니다.
- Daily 가격 요약의 fresh/stale/불완전/이상치 수를 확인합니다.
- 이상치 또는 가격 누락이 있으면 가능한 범위에서 해당 `priceSnapshot`과 `sourceNote`를 1회 보정한 뒤 `npm run check:data`를 다시 실행합니다.
- 가격 품질 문제 때문에 전체 분석을 중단하지는 않되, 보정 후에도 남는 문제는 최종 응답에 짧게 남깁니다.
- 최종 응답의 데이터 품질 요약은 `Daily 가격 fresh A/B, Results 근거 fresh C/D, mirror clean` 형식으로 씁니다.
- 커밋 후 루트에서 `npm run check:mirror`를 실행해 `github-pages-root` 미커밋 상태를 확인합니다.
- 이번 실행 data 파일 외 변경이 남아 있으면 되돌리지 말고 최종 응답에 경고로 표시합니다.
- push 시도 후 성공 여부를 최종 응답에 `push 완료` 또는 `push 실패, 로컬 커밋 유지`로 표시합니다.
...
- 오늘 있었던 일을 길게 요약하기보다, 앞으로의 촉매와 상대강도 지속 가능성을 먼저 적습니다.
...
- 섹터 카드는 "리뷰"보다 "포지셔닝" 관점으로 작성합니다.
- 각 섹터의 `thesis`는 가능하면 아래 흐름을 따릅니다:
  - 최근 사실 1문장
  - 앞으로 3~10거래일 동안 더 강하거나 더 약해질 이유 1~2문장
  - 무엇이 틀리면 이 판단을 접어야 하는지 1문장
- 각 섹터의 핵심 문장은 "왜 오늘 올랐는가"보다 "왜 아직 덜 끝났는가 / 왜 더 악화될 수 있는가"를 우선 설명합니다.
- 오늘 장 리뷰성 문장은 섹터당 1문장 이내로 제한하고, 나머지는 모두 선행 촉매/수급 지속성/무효화 조건 중심으로 씁니다.
- 각 유망 섹터는 "향후 1~3일", "향후 1~3주" 관점에서 모두 유효한지 내부적으로 점검한 뒤, 두 시간축 중 더 강한 쪽에 맞춰 `thesis`를 씁니다.
- 각 주의 섹터도 마찬가지로 단기 반등 리스크와 1~3주 약화 가능성을 함께 비교한 뒤 최종 판단합니다.
...
- `keyDrivers`와 `headwinds`는 단순 사실 나열보다 앞으로의 상대강도를 설명하는 항목 위주로 씁니다.
- `risks`와 `whatCouldImprove`는 반드시 "이 섹터 판단이 틀릴 수 있는 조건"을 포함합니다.
...
- `priceSnapshot` 값은 시장별 기본 가격 소스에서 확인 가능한 가장 최신 가격 정보를 기준으로 채웁니다.
- `priceDate`는 해당 가격 소스 페이지에 표시된 실제 기준일로 넣습니다.
- 가격 판단과 `priceSnapshot`은 시장별 기본 가격 소스 기준으로 맞춥니다.
- 가격 확인은 중요하지만 특정 종목이나 특정 페이지에서 전체 자동화가 멈추면 안 됩니다.
- 신규 편입, 상향, `smallCapIdeas` 후보는 가격 확인 우선순위를 가장 높게 두고, 가능하면 현재가, 전거래일 대비, 52주 최고/최저를 모두 확인합니다.
- 기존 `hold` 유지 후보와 하향/제외 후보는 기본 가격 소스 1회 확인으로 충분하며, 조회가 지연되면 추가 재시도보다 전체 분석 진행을 우선합니다.
- 미국 종목은 Yahoo Finance 종목 상세 페이지 `https://finance.yahoo.com/quote/{TICKER}/` 기준으로 확인합니다.
- 미국 종목은 Yahoo Finance에서 최신 가격을 못 찾았거나 값이 비어 있으면 Naver Finance 해외/미국 종목 페이지도 1회 추가 확인합니다.
- Yahoo와 Naver를 둘 다 봤는데 최신 가격이 불명확하거나 값이 서로 충돌하면, 해당 숫자는 추정하지 말고 비운 뒤 `sourceNote`에 두 소스를 확인했음을 남깁니다.
...
- 한국 종목 Naver Finance 페이지가 지연되거나 응답이 불안정하면 같은 종목 URL은 최대 2회까지만 시도합니다.
- 2회 확인 뒤에도 현재가 또는 기준일을 안정적으로 확인하지 못하면 해당 종목을 더 붙잡지 말고 숫자 필드를 비운 뒤 `sourceNote`에 "Naver Finance 조회 지연/불안정으로 최신 가격 미확보"처럼 짧게 남깁니다.
- 가격 확인 실패 때문에 JSON 생성, 섹터 선정, 배포 동기화를 중단하지 않습니다.
- 가격 스냅샷이 비어 있는 신규/상향 후보는 원칙적으로 신규 편입이나 상향을 피하고, 대체 후보를 찾거나 `hold`/제외로 보수 처리합니다.
...
- 시장별 기본 가격 소스에서 최신 가격 정보를 끝내 찾지 못한 항목은 추정해서 채우지 말고 해당 숫자 필드를 비웁니다.
- 일부 항목만 찾았으면 찾은 값만 넣고, 못 찾은 값은 비운 뒤 `sourceNote`에 누락 이유를 짧게 남깁니다.
- 주말 또는 휴장일에는 현재 캘린더 날짜와 가격 기준일이 다를 수 있으므로, 시장별 기본 가격 소스가 표시하는 마지막 거래일 가격을 사용합니다.
- 마지막 거래일 가격이 분석 기준일보다 5일 이내이면 stale로 보지 않고, `sourceNote`에 "마지막 거래일 기준"을 명시합니다.
- 주말/휴장일에는 `priceDate`를 분석 실행일로 바꾸지 말고 실제 source quote date를 유지합니다.
- 주말/휴장일 분석 문구는 새 가격 움직임을 가장하지 말고 마지막 거래일 이후 뉴스, 선물/환율, 실적/정책 이벤트, 다음 개장 포지셔닝을 중심으로 씁니다.
...
- 각 `smallCapIdeas` 항목에는 `followThroughNote`를 함께 넣습니다.
- `followThroughNote`는 "며칠째 올랐거나 며칠째 추적 중이어도 왜 아직 보는지", "추가 상승 여지가 남았는지 아니면 추격 리스크가 더 큰지"를 분명하게 설명하는 한두 문장입니다.
- 전일 `smallCapIdeas`에 있었고 핵심 논리가 아직 유효한 종목은 가급적 하루 만에 교체하지 말고, 다음 실행에서도 다시 포함해 가격 스냅샷이 이어지도록 합니다.
- 전일 `smallCapIdeas` 종목을 제외할 때는 가격 부담, 실적/수급 변화, 논리 훼손 등 제외 이유가 분명할 때만 교체합니다.
...
- 종목 `rationale`도 "오늘 올랐다/내렸다"보다 "섹터 시나리오가 이어질 때 왜 이 종목이 더 유리하거나 취약한지"를 우선 씁니다.
- `changesSinceYesterday`는 단순 등락 설명보다 포지셔닝 변경 이유를 적습니다.
- `reasons` 블록도 복기 메모가 아니라, 오늘 판단을 밀어주는 선행 근거와 향후 체크 포인트를 압축하는 용도로 씁니다.
- 시장별 기본 가격 소스 페이지에 노출된 최신 가격을 1회 확인하는 것으로 충분하며, 오래된 값이나 다른 사이트 값으로 대체하지 않습니다.
- 5일 이상 오래된 가격밖에 없다면 "최신값을 확보하지 못한 것"으로 간주하고, 오래된 숫자로 설명 문구를 정당화하지 않습니다.
...
- 엄격한 필터를 적용한 결과 적합한 중소형주가 부족하면 `smallCapIdeas`는 1~2개만 작성해도 됩니다.
...
- 섹터를 고를 때 가장 먼저 묻는 질문은 "향후 며칠 동안 자금이 더 들어올 이유가 남아 있는가"입니다.
- 이미 강했던 섹터라도 다음 촉매가 없고 기대가 대부분 가격에 반영됐다면 순위를 낮춥니다.
- 반대로 아직 가격 반영이 덜 됐지만 실적, 정책, 수급, 공급망 변화가 이어질 수 있는 섹터는 더 공격적으로 상향 검토합니다.
- 유망 섹터는 "추가 자금 유입 가능성", 주의 섹터는 "추가 디레이팅 가능성"을 기준으로 고릅니다.
- 매 실행마다 최소 2개의 challenger sector를 내부적으로 비교한 뒤, 왜 상위 3개에 못 들었는지도 판단하고 넘어갑니다.
  ...
  "smallCapIdeas": [
    {
      ...
      "whyNow": "지금 보는 이유 한 줄",
      "followThroughNote": "며칠째 추적하거나 최근 반등이 이어져도 왜 아직 보는지 설명",
      "priceSnapshot": {
        "priceDate": "YYYY-MM-DD",
        "currentPrice": 0,
        "previousCloseChangePct": 0,
...
12. 커밋이 생성됐으면 루트에서 `npm run push:pages`를 실행합니다.
