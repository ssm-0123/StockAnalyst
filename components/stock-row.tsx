"use client";

import { useState } from "react";
import { ArrowDownRight, ArrowUpRight, ChevronDown, Minus, Sparkles, X } from "lucide-react";

import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { getMarketBadgeVariant, getMarketLabel } from "@/lib/market";
import { liveQuotesEnabled, withBasePath } from "@/lib/site";
import { cn } from "@/lib/utils";
import { StockIdea, StockPriceSnapshot } from "@/lib/types";

const badgeMap = {
  NEW: { label: "신규", icon: Sparkles, className: "bg-emerald-50 text-emerald-700" },
  HOLD: { label: "유지", icon: Minus, className: "bg-slate-100 text-slate-600" },
  UPGRADE: { label: "상향", icon: ArrowUpRight, className: "bg-emerald-50 text-emerald-700" },
  DOWNGRADE: { label: "하향", icon: ArrowDownRight, className: "bg-rose-50 text-rose-700" },
  REMOVED: { label: "제외", icon: X, className: "bg-rose-50 text-rose-700" },
} as const;

const quoteCache = new Map<string, StockPriceSnapshot>();
const inflightCache = new Map<string, Promise<StockPriceSnapshot | null>>();

function normalizeStatus(stock: StockIdea) {
  if (stock.isNew || stock.change === "new") {
    return "NEW" as const;
  }

  if (stock.change === "no-change" || stock.change === "hold") {
    return "HOLD" as const;
  }

  if (stock.change === "up" || stock.change === "upgrade") {
    return "UPGRADE" as const;
  }

  if (stock.change === "down" || stock.change === "downgrade") {
    return "DOWNGRADE" as const;
  }

  return "REMOVED" as const;
}

function formatPrice(value?: number, market?: StockIdea["market"], currency?: "KRW" | "USD") {
  if (typeof value !== "number") {
    return "업데이트 예정";
  }

  return new Intl.NumberFormat(market === "KR" ? "ko-KR" : "en-US", {
    style: "currency",
    currency: currency ?? (market === "KR" ? "KRW" : "USD"),
    maximumFractionDigits: market === "KR" ? 0 : 2,
  }).format(value);
}

function formatPercent(value?: number) {
  if (typeof value !== "number") {
    return "업데이트 예정";
  }

  const sign = value > 0 ? "+" : "";
  return `${sign}${value.toFixed(2)}%`;
}

async function fetchQuoteSnapshot(stock: StockIdea) {
  const cacheKey = `${stock.market}:${stock.quoteSymbol ?? stock.ticker}`;

  if (quoteCache.has(cacheKey)) {
    return quoteCache.get(cacheKey) ?? null;
  }

  if (inflightCache.has(cacheKey)) {
    return inflightCache.get(cacheKey) ?? null;
  }

  const promise = fetch(
    `${withBasePath(`/api/quotes?symbol=${encodeURIComponent(stock.ticker)}&market=${encodeURIComponent(stock.market)}${
      stock.quoteSymbol ? `&quoteSymbol=${encodeURIComponent(stock.quoteSymbol)}` : ""
    }`)}`,
  )
    .then(async (response) => {
      if (!response.ok) {
        throw new Error("quote_fetch_failed");
      }

      const payload = (await response.json()) as StockPriceSnapshot;
      quoteCache.set(cacheKey, payload);
      return payload;
    })
    .catch(() => null)
    .finally(() => {
      inflightCache.delete(cacheKey);
    });

  inflightCache.set(cacheKey, promise);
  return promise;
}

export function StockRow({ stock, caution = false }: { stock: StockIdea; caution?: boolean }) {
  const status = normalizeStatus(stock);
  const config = badgeMap[status];
  const Icon = config.icon;
  const [isOpen, setIsOpen] = useState(false);
  const [liveSnapshot, setLiveSnapshot] = useState<StockPriceSnapshot | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [loadFailed, setLoadFailed] = useState(false);

  const analysisSnapshot = stock.priceSnapshot;
  const analysisCurrency = analysisSnapshot?.currency ?? (stock.market === "KR" ? "KRW" : "USD");
  const liveCurrency = liveSnapshot?.currency ?? (stock.market === "KR" ? "KRW" : "USD");
  const hasAnalysisMetrics =
    analysisSnapshot?.currentPrice != null ||
    analysisSnapshot?.previousCloseChangePct != null ||
    analysisSnapshot?.week52High != null ||
    analysisSnapshot?.week52Low != null;
  const hasLiveMetrics =
    liveSnapshot?.currentPrice != null ||
    liveSnapshot?.previousCloseChangePct != null ||
    liveSnapshot?.week52High != null ||
    liveSnapshot?.week52Low != null;

  async function handleToggle(open: boolean) {
    setIsOpen(open);
  }

  async function handleRefreshLivePrice() {
    if (!liveQuotesEnabled || liveSnapshot || isLoading) {
      return;
    }

    setIsLoading(true);
    setLoadFailed(false);
    const snapshot = await fetchQuoteSnapshot(stock);
    if (snapshot) {
      setLiveSnapshot(snapshot);
    } else {
      setLoadFailed(true);
    }
    setIsLoading(false);
  }

  return (
    <details
      open={isOpen}
      onToggle={(event) => handleToggle((event.currentTarget as HTMLDetailsElement).open)}
      className="group rounded-2xl border border-slate-200/80 bg-white/70"
    >
      <summary className="flex cursor-pointer list-none items-start justify-between gap-3 px-3 py-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <span className="text-sm font-semibold tracking-[0.18em] text-slate-900">{stock.ticker}</span>
            <Badge variant={getMarketBadgeVariant(stock.market)} className="px-2 py-0.5 text-[10px]">
              {getMarketLabel(stock.market)}
            </Badge>
            <p className="truncate text-sm font-medium text-slate-500">{stock.companyName}</p>
          </div>
          <p className={cn("mt-1 text-sm leading-5 text-slate-600", caution && "text-slate-700")}>
            {stock.rationale}
          </p>
          <p className="mt-2 text-[11px] font-medium uppercase tracking-[0.2em] text-slate-400">
            클릭해서 가격 정보 보기
          </p>
        </div>
        <div className="flex items-start gap-2">
          <div
            className={cn(
              "inline-flex shrink-0 items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-semibold",
              config.className,
            )}
          >
            <Icon className="size-3.5" />
            {config.label}
          </div>
          <div className="rounded-full bg-slate-100 p-1.5 text-slate-500 transition-transform group-open:rotate-180">
            <ChevronDown className="size-3.5" />
          </div>
        </div>
      </summary>
      <div className="border-t border-slate-200/80 px-3 py-3">
        <div className="space-y-4">
          <div>
            <div className="mb-2 flex items-center justify-between gap-3">
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">
                분석 시점 스냅샷
              </p>
              <span className="text-xs text-slate-500">{analysisSnapshot?.priceDate ?? "업데이트 예정"}</span>
            </div>
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <div className="rounded-2xl border border-slate-200/80 bg-slate-50/70 p-3">
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">현재가</p>
                <p className="mt-1 text-sm font-semibold text-slate-900">
                  {formatPrice(analysisSnapshot?.currentPrice, stock.market, analysisCurrency)}
                </p>
              </div>
              <div className="rounded-2xl border border-slate-200/80 bg-slate-50/70 p-3">
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">
                  전거래일 대비
                </p>
                <p
                  className={cn(
                    "mt-1 text-sm font-semibold",
                    typeof analysisSnapshot?.previousCloseChangePct === "number"
                      ? analysisSnapshot.previousCloseChangePct >= 0
                        ? "text-emerald-700"
                        : "text-rose-700"
                      : "text-slate-900",
                  )}
                >
                  {formatPercent(analysisSnapshot?.previousCloseChangePct)}
                </p>
              </div>
              <div className="rounded-2xl border border-slate-200/80 bg-slate-50/70 p-3">
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">52주 최고</p>
                <p className="mt-1 text-sm font-semibold text-slate-900">
                  {formatPrice(analysisSnapshot?.week52High, stock.market, analysisCurrency)}
                </p>
              </div>
              <div className="rounded-2xl border border-slate-200/80 bg-slate-50/70 p-3">
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">52주 최저</p>
                <p className="mt-1 text-sm font-semibold text-slate-900">
                  {formatPrice(analysisSnapshot?.week52Low, stock.market, analysisCurrency)}
                </p>
              </div>
            </div>
            <div className="mt-3 flex flex-wrap gap-3 text-xs text-slate-500">
              <span>{analysisSnapshot?.sourceNote ?? "자동화가 저장한 분석 시점 가격입니다."}</span>
            </div>
            {!hasAnalysisMetrics ? (
              <div className="mt-3 rounded-2xl border border-dashed border-slate-200 bg-white/80 px-3 py-2 text-sm text-slate-500">
                아직 분석 시점 가격 스냅샷 데이터가 없습니다.
              </div>
            ) : null}
          </div>

          <div className="rounded-2xl border border-cyan-100 bg-cyan-50/60 p-3">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-cyan-700">
                  현재 API 기준 가격
                </p>
                <p className="mt-1 text-xs text-slate-600">
                  {liveQuotesEnabled
                    ? "분석 시점 숫자와 지금 숫자가 다를 수 있습니다."
                    : "GitHub Pages 배포는 정적 사이트라 현재 API 조회가 비활성화됩니다."}
                </p>
              </div>
              <Button
                type="button"
                size="sm"
                variant="secondary"
                onClick={handleRefreshLivePrice}
                disabled={isLoading || !liveQuotesEnabled}
              >
                {isLoading ? "불러오는 중" : hasLiveMetrics ? "갱신됨" : "현재가 불러오기"}
              </Button>
            </div>
            {hasLiveMetrics ? (
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                <div className="rounded-2xl border border-cyan-100 bg-white/80 p-3">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">현재가</p>
                  <p className="mt-1 text-sm font-semibold text-slate-900">
                    {formatPrice(liveSnapshot?.currentPrice, stock.market, liveCurrency)}
                  </p>
                </div>
                <div className="rounded-2xl border border-cyan-100 bg-white/80 p-3">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">전거래일 대비</p>
                  <p
                    className={cn(
                      "mt-1 text-sm font-semibold",
                      typeof liveSnapshot?.previousCloseChangePct === "number"
                        ? liveSnapshot.previousCloseChangePct >= 0
                          ? "text-emerald-700"
                          : "text-rose-700"
                        : "text-slate-900",
                    )}
                  >
                    {formatPercent(liveSnapshot?.previousCloseChangePct)}
                  </p>
                </div>
                <div className="rounded-2xl border border-cyan-100 bg-white/80 p-3">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">52주 최고</p>
                  <p className="mt-1 text-sm font-semibold text-slate-900">
                    {formatPrice(liveSnapshot?.week52High, stock.market, liveCurrency)}
                  </p>
                </div>
                <div className="rounded-2xl border border-cyan-100 bg-white/80 p-3">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">52주 최저</p>
                  <p className="mt-1 text-sm font-semibold text-slate-900">
                    {formatPrice(liveSnapshot?.week52Low, stock.market, liveCurrency)}
                  </p>
                </div>
              </div>
            ) : (
              <div className="rounded-2xl border border-dashed border-cyan-100 bg-white/80 px-3 py-2 text-sm text-slate-500">
                {!liveQuotesEnabled
                  ? "이 배포에서는 저장된 스냅샷만 표시됩니다."
                  : loadFailed
                  ? "가격 API 조회에 실패했습니다. 잠시 후 다시 시도해 주세요."
                  : "버튼을 누르면 현재 API 기준 가격을 불러옵니다."}
              </div>
            )}
            {hasLiveMetrics ? (
              <div className="mt-3 flex flex-wrap gap-3 text-xs text-slate-500">
                <span>기준일 {liveSnapshot?.priceDate ?? "업데이트 예정"}</span>
                <span>{liveSnapshot?.sourceNote ?? "현재 API 기준"}</span>
              </div>
            ) : null}
          </div>
        </div>
      </div>
    </details>
  );
}
